...
class NotificationDispatcher(val context: Context, val repository: Repository) {
    ...
    fun dispatch(subscription: Subscription, notification: Notification) {
        ...
        try {
            System.out.println("Sending to glass")
            val dateFormat: DateFormat = SimpleDateFormat("yyyy/MM/dd HH:mm:ss")
            val date: Date = Date()
            val rand = Math.random() * 10000
            var notitle: String
            notitle = notification.title
            if (notification.title == "") {
                notitle = subscription.topic
            }
            val envelope: Proto.Envelope = GlassMessagingUtil.createTimelineMessage(
                "<article> <section><p style=\"font-size:64px\">subject</p> <p class=\"text-auto-size\">text</p> </section> </article>".replace("subject", notitle).replace("text", notification.message), rand.toString())
            the.joeglass.JoeMessageUtil.send(envelope)
            System.out.println("Sent envelope " + envelope.toString())
        } catch (e: Exception) {
            e.printStackTrace()
        }
